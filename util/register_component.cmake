...
function(register_component component_name)
    get_filename_component(component_code_dir ${CMAKE_CURRENT_LIST_FILE} DIRECTORY)
    #get_filename_component(component_name ${component_code_dir} NAME)
    message(STATUS "[register component: ${component_name} ], path:${component_code_dir}")
    ...
    foreach(include_dir ${ADD_INCLUDE})
        get_filename_component(abs_dir ${include_dir} ABSOLUTE BASE_DIR ${component_code_dir})
    ...
    if(ADD_STATIC_LIB)
        foreach(lib ${ADD_STATIC_LIB})
            if(NOT EXISTS "${lib}")
                prepend(lib_full "${component_code_dir}/" ${lib})
